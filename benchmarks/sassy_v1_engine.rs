// External comparator only. IUPAC supports the v1 multi-pattern method.
// The v1 API honors without_trace; the batched v2
// API in pinned 0.2.6 does not pass that flag into its pattern-tiling engine.
mod engine {
    use sassy::{Searcher, profiles::Iupac};
    pub const NAME: &str = "sassy-0.2.6-v1-iupac-without-trace";
    pub struct Prepared { searcher: Searcher<Iupac>, patterns: Vec<Vec<u8>> }
    impl Prepared {
        pub fn new(patterns: &[Vec<u8>]) -> Self {
            Self { searcher: Searcher::<Iupac>::new_fwd().without_trace(),
